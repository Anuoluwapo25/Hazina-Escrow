 ...
 * or a seller JWT — every wallet on the marketplace (seller or curator) is the
 * same identity, scoped by the same `sellerWallet` JWT claim. When a JWT is
 * used, `walletField` in the request body must match the JWT's wallet claim,
 * so a curator can only build a bundle they can prove ownership of.
 */
